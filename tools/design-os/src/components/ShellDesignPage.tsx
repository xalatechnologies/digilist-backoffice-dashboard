...
export function ShellDesignFullscreen() {
  ...
  const ShellPreviewComponent = useMemo(() => {
    ...
    try {
      return React.lazy(() => {
        return shellPreviewLoader().catch((error) => {
          console.error('Failed to load ShellPreview:', error)
          // Return a fallback component
          return {
            default: () => (
              <div className="h-screen flex items-center justify-center bg-background">
                <div className="text-center space-y-2">
                  <p className="text-stone-600 dark:text-stone-400">Failed to load shell preview.</p>
                  <p className="text-xs text-stone-500 dark:text-stone-500">
                    Please restart the dev server if you just installed dependencies.
                  </p>
                </div>
              </div>
            ),
